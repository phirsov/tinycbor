...
struct Token
{
    ...
    enum {None, Array, Map, Int, Double, String, ClosingBracket} type;
...
static size_t skipatomtoken(const char *str, const char *token) {
    size_t len = strlen(token);

    if (len && !strncmp(str, token, len)) {
        return len;
    };
...
static Token *firsttoken(const char *str, const char **pend)
{
    ...
    } else if ((skip = skipatomtoken(str, "Array["))) {
        type = Array;

    } else if ((skip = skipatomtoken(str, "Map["))) {
        type = Map;

    } else if ((skip = skipatomtoken(str, "]"))) {
        type = ClosingBracket;
        ...
        encoder_create_container_fn create_container)
{
    ...
    err = create_container(&ctx->encoder, &nestedctx.encoder,
            CborIndefiniteLength);

    if (err != CborNoError) {
        complainencode(err, token->lineno);

    } else if (encoderecursive(&nestedctx)) {
        ...
        err = cbor_encoder_close_container(&ctx->encoder,
            &nestedctx.encoder),
        err != CborNoError
    ) {
        complainencode(err, token->lineno);
